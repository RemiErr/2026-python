...
def read_zip_summary(zip_path: Path):
    ...
    with zipfile.ZipFile(zip_path) as archive:
        for info in sorted(archive.infolist(), key=lambda item: item.filename):
            ...
            # 檔名前三碼就是年度，例如 109、110、114。
            year = info.filename[:3]

            # utf-8-sig 可以把 Excel 常見的 BOM 一起處理掉。
            text = archive.read(info).decode("utf-8-sig")
            rows = list(csv.reader(io.StringIO(text)))
            header = rows[0]
            data = rows[1:]

            # 找到欄位位置後，就可以用 Counter 很快完成統計。
...
def make_report(summary: dict) -> str:
    """
    把統計結果整理成 Markdown 表格。
    這樣之後不管要印到畫面、寫成檔案，或貼到報告裡都很方便。
    """
    lines = [
        "# 6 屆新生概況報告",
        "",
...
def main():
    ...
    # 用 TemporaryDirectory 示範「先在沙箱產生檔案，再自動清掉」的做法。
    with tempfile.TemporaryDirectory() as temp_dir:
        temp_root = Path(temp_dir)
        snap_path = temp_root / "summary.pkl"
        report_path = temp_root / "report.md"

        # pickle 適合保存 Python 物件，之後可以直接讀回 Counter 與 dict。
